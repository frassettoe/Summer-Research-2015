...
class Edge:
    ...
    def calculateEdgeCurvature(self, listOfTetrahedra):
        ...
        for i in range(len(self.tetrahedraEdgeIsIn)):  # for each tetrahedran the edge is in
            tetLocation = self.tetrahedraEdgeIsIn[
                i]  # find the the location of that tetrahedron in the list of tetrahedra
            ...
            singleDi = listOfTetrahedra[tetLocation].dihedralanglelist[
                listOfTetrahedra[tetLocation].edgesintetrahedron.index([self.vertex1, self.vertex2])]
    ...
    def calculateEdgeLengthStar(self, listOfTetrahedra):
        ...
        for i in range(len(self.tetrahedraEdgeIsIn)):
            ...
            kLoc = list[0]
            lLoc = list[1]
            hijk = listOfTetrahedra[tetLocation].faceList[listOfTetrahedra[tetLocation].vertexList.index(lLoc)].hList[
                listOfTetrahedra[tetLocation].faceList[
                    listOfTetrahedra[tetLocation].vertexList.index(lLoc)].vertexList.index(kLoc)]
            hijkl = listOfTetrahedra[tetLocation].tetCenDisList[listOfTetrahedra[tetLocation].vertexList.index(lLoc)]
            hijl = listOfTetrahedra[tetLocation].faceList[listOfTetrahedra[tetLocation].vertexList.index(kLoc)].hList[
                listOfTetrahedra[tetLocation].faceList[
                    listOfTetrahedra[tetLocation].vertexList.index(kLoc)].vertexList.index(lLoc)]
...
class face:
    def getAngle(self, c, a,
                 b):  # uses law of cosines to find the angle given three sides of a triangle, returns cosine of angle
        temp = a ** 2 + b ** 2 - c ** 2
        temp = temp / (2 * a * b)
        return temp
    ...
    def __init__(self, edgeTable, vertex1=1, vertex2=2, vertex3=3):
        ...
        self.edgeLength = [edgeTable[vertex2][vertex3].edgelength, edgeTable[vertex1][vertex3].edgelength,
                           edgeTable[vertex1][vertex2].edgelength]  # edgeLength[i] is opposite to vertex i-1
        self.angleList = [math.acos(self.getAngle(self.edgeLength[0], self.edgeLength[1], self.edgeLength[2])),
                          math.acos(self.getAngle(self.edgeLength[1], self.edgeLength[0], self.edgeLength[2])),
                          math.acos(self.getAngle(self.edgeLength[2], self.edgeLength[1], self.edgeLength[0]))]
        self.hList = [self.getTriCenDis(self.edgeLength[2], self.edgeLength[0], self.angleList[1]),
                      self.getTriCenDis(self.edgeLength[0], self.edgeLength[1], self.angleList[2]),
                      self.getTriCenDis(self.edgeLength[1], self.edgeLength[2], self.angleList[0])]
...
class Tetrahedron:
    ...
    def checkLegalTetrahedron(self, tableOfEdges):
        ...
        det = (-2) * (edge1 ** 4) * (edge6 ** 2) - (2) * (edge1 ** 2) * (edge2 ** 2) * (edge4 ** 2) + (2) * (
        edge1 ** 2) * (edge2 ** 2) * (edge5 ** 2) + (2) * (edge1 ** 2) * (edge2 ** 2) * (edge6 ** 2) + (2) * (
        edge1 ** 2) * (edge3 ** 2) * (edge4 ** 2) - (2) * (edge1 ** 2) * (edge3 ** 2) * (edge5 ** 2) + (2) * (
        edge1 ** 2) * (edge3 ** 2) * (edge6 ** 2) + (2) * (edge1 ** 2) * (edge4 ** 2) * (edge6 ** 2) + (2) * (
        edge1 ** 2) * (edge5 ** 2) * (edge6 ** 2) - (2) * (edge1 ** 2) * (edge6 ** 4) - (2) * (edge2 ** 4) * (
        edge5 ** 2) + (2) * (edge2 ** 2) * (edge3 ** 2) * (edge4 ** 2) + (2) * (edge2 ** 2) * (edge3 ** 2) * (
        edge5 ** 2) - (2) * (edge2 ** 2) * (edge3 ** 2) * (edge6 ** 2) + (2) * (edge2 ** 2) * (edge4 ** 2) * (
        edge5 ** 2) - (2) * (edge2 ** 2) * (edge5 ** 4) + (2) * (edge2 ** 2) * (edge5 ** 2) * (edge6 ** 2) - (2) * (
        edge3 ** 4) * (edge4 ** 2) - (2) * (edge3 ** 2) * (edge4 ** 4) + (2) * (edge3 ** 2) * (edge4 ** 2) * (
        edge5 ** 2) + (2) * (edge3 ** 2) * (edge4 ** 2) * (edge6 ** 2) - (2) * (edge4 ** 2) * (edge5 ** 2) * (
        edge6 ** 2)
        if det <= 0:
    ...
    # change log:
    def getAngle(self, c, a,
                 b):  # Uses law of cosines to return the cosine of an angle given three edge lengths of a triangle
        temp = a ** 2 + b ** 2 - c ** 2
        temp = temp / (2 * a * b)
        return temp
    ...
    def calculateDihedralAngles(self, tableOfEdges):  # Used to find all the dihedral angles of a tetrahedron
        self.dihedralanglelist = []
        edge12 = tableOfEdges[self.vertex1][
            self.vertex2].edgelength  # gets edge lengths in a form easier for the coder to understand
        edge13 = tableOfEdges[self.vertex1][
            self.vertex3].edgelength  # this reduced the risk of mistyping the order of edges in calDiAngle calls
        ...
        self.dihedralanglelist.append(
            math.acos(self.calDiAngle(edge12, edge13, edge14, edge23, edge24, edge34)))  # finds each dihedral angle
...
class backgroundMetricClass:  # need to change so conformal variations are always 0.
    ...
    def fillEdgeTable(self, listOfTetrahedra, tableOfEdges, listOfEdges, fileName="backgroundMetric.txt"):
        ...
        for i in range(len(listOfTetrahedra)):  # for each tetrahedra
            for j in range(len(listOfTetrahedra[i].edgesintetrahedron)):  # look at every edge in the tetrahedra
                ...
                if tableOfEdges[listOfTetrahedra[i].edgesintetrahedron[j][0]][
                    listOfTetrahedra[i].edgesintetrahedron[j][1]] == 0:
                    # Assigns edge its name
                    lengthIndex = listOfEdges.index(
                        [listOfTetrahedra[i].edgesintetrahedron[j][0], listOfTetrahedra[i].edgesintetrahedron[j][1]])
                    tableOfEdges[listOfTetrahedra[i].edgesintetrahedron[j][0]][
                        listOfTetrahedra[i].edgesintetrahedron[j][1]] = Edge(
                        listOfTetrahedra[i].edgesintetrahedron[j][0], listOfTetrahedra[i].edgesintetrahedron[j][1],
                    ...
                    tableOfEdges[listOfTetrahedra[i].edgesintetrahedron[j][0]][
                        listOfTetrahedra[i].edgesintetrahedron[j][1]].tetrahedraEdgeIsIn.append(i)
    ...
    def createTetrahedraList(self, primalList):  # creates list of tetrahedra in metric
        ...
        for i in range(len(primalList)):
            self.tetrahedralist.append(
                Tetrahedron(primalList[i][0], primalList[i][1], primalList[i][2], primalList[i][3]))
    ...
    def findTotalEdgeLength(self, tableOfEdges):
        ...
        for i in range(1, self.vertexNumber + 1):
            for j in range(len(tableOfEdges[0])):  # looks at every entry in edge table
                if tableOfEdges[j][
                    i] != 0:  # 0 indicates no edge present, so will only add edge length if edge is present
    ...
    def calculateVertexCurvature(self, vertex, tableOfEdges):
        ...
        for i in range(len(tableOfEdges[0])):  # looks at every edge associated with the given vertex
            if tableOfEdges[vertex][i] != 0:  # checks both possible forms of edge name (aka (i,j) and (j,i))
                vertexCurvature = vertexCurvature + tableOfEdges[vertex][
                    i].edgecurvature  # adds edge curvature as required by vertex curvature definition
    ...
    def checkLCSC(self, tableOfEdges, error=.0001):
        ...
        for i in range(len(self.edgeList)):  # for each edge
            listOfLengths[self.edgeList[i][0]].append(self.edgetable[self.edgeList[i][0]][self.edgeList[i][
                1]].edgelength)  # adds edge length to the two verticies are connected
            listOfLengths[self.edgeList[i][1]].append(
                self.edgetable[self.edgeList[i][0]][self.edgeList[i][1]].edgelength)
            # for i in range(self.vertexNumber):
            #     print(i,self.calculateVertexCurvature(i,tableOfEdges),self.LEHR * L)
        for i in range(1, self.vertexNumber + 1):
            if math.fabs(self.calculateVertexCurvature(i, tableOfEdges) - (self.LEHR * sum(listOfLengths[i]) / 2)) > (
            error):  # checks if LCSC is met
    ...
    def __init__(self, backgroundFile='backgroundMetric.txt', manifoldFile='manifoldExample.txt'):
        ...
        readFile.close()
        tetrahedron = [[int(i) for i in tetrahedron[j]] for j in
                       range(len(tetrahedron))]  # turns tetrahedron from str to int
        # start of calculations, end of file reading (in this function)
        self.vertexNumber = self.createTetrahedraList(
            tetrahedron)  # builds list of tetrahedra and sets number of verticies in metric
        self.createEdgeTable(self.edgetable, self.vertexNumber)  # builds empty edge table (all zeros)
        self.fillEdgeTable(self.tetrahedralist, self.edgetable, self.edgeList,
                           backgroundFile)  # places edges in edge table (replaces zeros with edges where needed)
        self.sumOfEdgesAtVertex = self.getEdgeSums(self.edgetable,
                                                   self.vertexNumber)  # finds the sum of edge lengths at each vertex
        illegalTetrahedrons = self.advancedCheckLegalTetrahedra(self.tetrahedralist,
                                                                self.edgetable)  # checks for legal tetradra
        if illegalTetrahedrons == True:  # if illegal found, further calculations cannot be done and program stops
            ...
            for i in range(len(self.tetrahedralist)):
                self.tetrahedralist[i].initalizeTriangles(
                    self.edgetable)  # finds information about triangles within tetrahedra (used in hessian calculations only?)
            ...
            for i in range(len(self.edgeList)):  # for each edge
                self.edgetable[self.edgeList[i][0]][self.edgeList[i][1]].calculateEdgeCurvature(
                    self.tetrahedralist)  # finds edge curvature
            ...
            for i in range(self.vertexNumber):
                self.vertexCurvatureList.append(self.calculateVertexCurvature(i + 1,
                                                                              self.edgetable))  # creates list of vertex curvatures (used for LCSC check?)
...
class metric:
    ...
    def advancedPrint(self, conVar):
        ...
        for i in range(len(self.background.tetrahedralist)):
            ...
            edge1 = self.edgeTable[self.background.tetrahedralist[i].vertexList[0]][
                self.background.tetrahedralist[i].vertexList[1]].edgelength
            edge2 = self.edgeTable[self.background.tetrahedralist[i].vertexList[0]][
                self.background.tetrahedralist[i].vertexList[2]].edgelength
            edge3 = self.edgeTable[self.background.tetrahedralist[i].vertexList[0]][
                self.background.tetrahedralist[i].vertexList[3]].edgelength
            edge4 = self.edgeTable[self.background.tetrahedralist[i].vertexList[1]][
                self.background.tetrahedralist[i].vertexList[2]].edgelength
            edge5 = self.edgeTable[self.background.tetrahedralist[i].vertexList[1]][
                self.background.tetrahedralist[i].vertexList[3]].edgelength
            edge6 = self.edgeTable[self.background.tetrahedralist[i].vertexList[2]][
                self.background.tetrahedralist[i].vertexList[3]].edgelength
            det = (-2) * (edge1 ** 4) * (edge6 ** 2) - (2) * (edge1 ** 2) * (edge2 ** 2) * (edge4 ** 2) + (2) * (
            edge1 ** 2) * (edge2 ** 2) * (edge5 ** 2) + (2) * (edge1 ** 2) * (edge2 ** 2) * (edge6 ** 2) + (2) * (
            edge1 ** 2) * (edge3 ** 2) * (edge4 ** 2) - (2) * (edge1 ** 2) * (edge3 ** 2) * (edge5 ** 2) + (2) * (
        ...
        for i in range(len(edgeLengthTable)):
            for j in range(len(edgeLengthTable[i])):
                if (edgeLengthTable[i][j] != 0):
                    edgeLengthTable[i][j].edgelength = math.exp(.5 * (convar[i - 1] + convar[j - 1])) * \
                                                       edgeLengthTable[i][j].edgelength
                    self.totalEdgeLength = self.totalEdgeLength + edgeLengthTable[i][j].edgelength / 2
                    self.sumOfEdgesAtVertexList[i - 1] = self.sumOfEdgesAtVertexList[i - 1] + edgeLengthTable[i][
                                                                                                  j].edgelength / 2
                    self.sumOfEdgesAtVertexList[j - 1] = self.sumOfEdgesAtVertexList[j - 1] + edgeLengthTable[i][
                                                                                                  j].edgelength / 2
...
def hessianSame(metric, vertex):
    ...
    partial = partial - metric.sumOfEdgesAtVertex[vertex - 1] * metric.vertexCurvatureList[vertex - 1] / (
    metric.totalLength ** 2)
...
def hessianDifferent(metric, vertex1, vertex2):
    if vertex1 > vertex2:
        lijStarOverlij = metric.edgetable[vertex2][vertex1].edgelengthStar / metric.edgetable[vertex2][
            vertex1].edgelength
        edgeCurvature = metric.edgetable[vertex2][vertex1].edgecurvature
        edgeLength = metric.edgetable[vertex2][vertex1].edgelength
    else:
        lijStarOverlij = metric.edgetable[vertex1][vertex2].edgelengthStar / metric.edgetable[vertex1][
            vertex2].edgelength
    ...
    partial = partial - metric.vertexCurvatureList[vertex2 - 1] * metric.sumOfEdgesAtVertex[vertex1 - 1] / (
    2 * metric.totalLength ** 2)
    partial = partial + metric.sumOfEdgesAtVertex[vertex1 - 1] * metric.LEHR * metric.sumOfEdgesAtVertex[
        vertex2 - 1] / (2 * metric.totalLength ** 2)
    partial = partial - metric.sumOfEdgesAtVertex[vertex2 - 1] * metric.vertexCurvatureList[vertex1 - 1] / (
    2 * metric.totalLength ** 2)
    return partial
...
def LCSCWalk(cListValueStart, cLIstValueEnd, stepSize, conVar=0):
    storage = str(0) + ".txt"
    LEHRList = []
    numberVertices = 5
    cList = [cListValueStart, 0, 0, 0, 0]
    conformalVariations = [0, 0, 0, 0, conVar]
    output = []
    seed = 56932684
    random.seed(seed)
    # seed=9865721
    ...
    while (cList[0] < cLIstValueEnd):
        modifyBackground(cList, base, "backgroundMetric.txt")
        conformalVariations = findLCSC(backgroundfile, triangulation, conformalVariations)
        temp = metric(backgroundfile, triangulation, conformalVariations)
        print("LCSC: " + str(temp.isLCSC))
        print("LEHR: " + str(temp.LEHR))
        print("Conformal Variation: " + str(conformalVariations[-1]))
        print("Background: " + str(cList[0]))
        cList[0] = cList[0] + stepSize
        output.append([cList[0], conformalVariations[-1]])
    return output


def findLCSC(back, tri, conVar):
    intial = 0
    metric0 = metric(back, tri, conVar)
    metric0.calLEHR(conVar)
    LEHR0 = metric0.LEHR
    stepSize = 10
    for i in range(100):
        conVar1 = copy.deepcopy(conVar)
        conVar1[-1] = conVar[-1] + stepSize
        metric1 = metric(back, tri, conVar1)
        metric1.calLEHR(conVar1)
        LEHR1 = metric1.LEHR
        conVarMinus1 = copy.deepcopy(conVar)
        conVarMinus1[-1] = conVar[-1] - stepSize
        metricMinus1 = metric(back, tri, conVarMinus1)
        metricMinus1.calLEHR(conVarMinus1)
        LEHRMinus1 = metricMinus1.LEHR
...
def main():
    print("Hello World!")
    results = LCSCWalk(2 * math.log(.8), 2 * math.log(1.25), .0009)
